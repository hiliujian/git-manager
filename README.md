...
一个现代化的 Git 可视化管理工具，提供直观的图形界面来管理 Git 仓库的各种操作。
...
**批量操作**：
- **全选/取消**：点击文件列表底部的"Select All"/"Unselect"按钮
- **还原所有**：点击"Revert All"按钮（谨慎使用）

### 推送到远程仓库

点击顶部工具栏的"Push"按钮，将本地提交推送到远程仓库。
...
程序默认使用以下端口：
- HTTP 服务器：7842（如被占用会自动递增）
- WebSocket 服务器：7843（如被占用会自动递增）

### 日志系统

程序会自动创建 `logs` 目录并记录日志：

- `git_manager_YYYYMMDD.log`：完整的运行日志（包含所有级别）
- `errors_YYYYMMDD.log`：错误日志（仅记录错误和严重问题）
...
- `Ctrl+S` / `Cmd+S`：保存当前编辑的文件
- `Shift+点击`：选择多行（在 Split 视图中）
